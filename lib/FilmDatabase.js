...
var fs = require("fs")
  ...
  , yearFilenameRegex = /((19|20)[0-9]{2})[^a-z0-9]/
...
FilmDatabase.prototype.analyse = function (file) {
  ...
  if (yearFilenameRegex.test(file.name)) {
    file.year = file.name.match(yearFilenameRegex)[1];
  }
  file.languages = [];
  ...
  tokens.forEach(function (token, i) {
    ...
    if (token.match(qualitiesRegex)) {
      ...
      //file.year = token;
      known = true;
    } else if (token.match(/German/)) {
      file.languages.push(token);
      known = true;
    } else if (token.match(/English/)) {
      file.languages.push(token);
      known = true;
    }
