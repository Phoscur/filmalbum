...
function $request(url, callback, cancel) {
  ...
  request(url, function (error, response, site) {
    ...
    if (error) {
      ...
      console.log('Error request', error, '/n/n/n');
...
function getCover(film, callback) {
  ...
  fs.exists(coversDir + film.title + ".jpg", function (exists) {
    if (exists) {
    ...
    $request("http://www.imdb.com" + href, function (error, window) {
      if (error) {
        return callback("Cover site scrape" + error, film);
      }
      ...
      if (src) {
        request(src).pipe(fs.createWriteStream(coversDir + film.title.replace('?', '') + ".jpg"))
          .on('close', function (error) {
            if (error) {
              callback('Cover loading error', error);
            }
            film.coverLoaded = true;
            callback(null, film);
          });
...
function scrapeIMDbTitleSite(film, $) {
  ...
  film.title = $('.header > span[itemprop="name"] :first').text().trim();
